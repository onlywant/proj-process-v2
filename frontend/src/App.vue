...
const exportColumns = [
  ...
  ['leadParticipatingUnits', '牵头参与单位'], ['totalAmount', '项目总金额'], ['contractAmount', '智芯合同金额'],
  ['source', '来源'], ['successDate', '申报成功时间'], ['zhixinRole', '智芯角色'],
  ['internalSupportDepartment', '内部支撑部门'], ['provincialSupportDepartment', '省公司支撑部门'],
  ['governmentUnit', '政府主管单位'], ['stage', '项目阶段'], ['health', '健康状态'],
...
async function chooseImport(event: Event) {
  ...
  importRows.value = rows.map(row => ({
    ...
    internalSupportDepartment: csvValue(importRowValue(row, '智芯内部支撑部门')), provincialSupportDepartment: csvValue(importRowValue(row, '省公司支撑部门')),
