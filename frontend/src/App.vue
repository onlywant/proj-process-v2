...
async function saveForm() {
  ...
  try {
    ...
    const payload = formMode.value === 'progress'
      ? { stage: form.stage, health: form.health, progress: form.progress, problemTags: form.problemTags, problemDescription: form.problemDescription, nextPlan: form.nextPlan, updateCycle: form.updateCycle, customCycleDays: form.customCycleDays, totalAmount: form.totalAmount, contractAmount: form.contractAmount, successDate: form.successDate, note: note.value, version: form.version }
...
function openResetPassword(target: User) { resetTarget.value = target; resetPassword.value = ''; resetPasswordConfirm.value = ''; resetPasswordError.value = '' }
...
async function resetUserPassword() {
  if (!resetTarget.value || resettingPassword.value) return
  resetPasswordError.value = ''
  if (resetPassword.value.length < 8) { resetPasswordError.value = '密码至少需要8位'; return }
  if (resetPassword.value !== resetPasswordConfirm.value) { resetPasswordError.value = '两次输入的新密码不一致'; return }
  const target = resetTarget.value
  resettingPassword.value = true
  try {
    await api(`/api/users/${target.id}`, { method: 'PUT', body: JSON.stringify({ password: resetPassword.value }) })
    resettingPassword.value = false
    closeResetPassword()
    if (target.id === user.value?.id) {
      logout()
      error.value = '密码已重置，请使用新密码重新登录'
    } else {
      error.value = '密码已重置，该用户需使用新密码重新登录'
    }
  } catch (e) {
    resetPasswordError.value = (e as Error).message
  } finally {
    resettingPassword.value = false
  }
}
...
<template>
  ...
  <div v-else class="app-shell">
    ...
    <div v-if="resetTarget" class="modal-backdrop"><form class="modal small-modal" role="dialog" aria-modal="true" aria-labelledby="reset-password-title" @submit.prevent="resetUserPassword"><button type="button" class="close-button" aria-label="关闭" :disabled="resettingPassword" @click="closeResetPassword">×</button><div class="section-kicker">用户管理</div><h2 id="reset-password-title">重置密码</h2><p class="muted">为 {{ resetTarget.name }}（{{ resetTarget.username }}）设置新密码。重置后，该用户需重新登录。</p><label>新密码<input v-model="resetPassword" type="password" minlength="8" autocomplete="new-password" placeholder="至少8位" :disabled="resettingPassword" required /></label><label>确认新密码<input v-model="resetPasswordConfirm" type="password" minlength="8" autocomplete="new-password" :disabled="resettingPassword" required /></label><p v-if="resetPasswordError" class="form-error" role="alert">{{ resetPasswordError }}</p><div class="modal-actions"><button type="button" :disabled="resettingPassword" @click="closeResetPassword">取消</button><button type="submit" class="primary" :disabled="resettingPassword">{{ resettingPassword ? '重置中...' : '确认重置' }}</button></div></form></div>
